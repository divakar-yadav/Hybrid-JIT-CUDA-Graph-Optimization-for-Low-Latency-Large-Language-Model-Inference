#!/usr/bin/env python3
"""
Case 4: JIT + Async Rolling CUDA Graph Client (Context Creator Process)
Same as Case 2 client but connects to JIT + CUDA graph server
"""
...
class JITAsyncCUDAClient:
    """Client for JIT + async CUDA graph inference via IPC"""
    
    def __init__(self, socket_path: str = "/tmp/jit_async_cuda_graph.sock"):
...
def run_case4_benchmark(
    socket_path: str = "/tmp/jit_async_cuda_graph.sock",
    prompt_lengths: List[int] = [10, 50, 100, 150, 200, 250, 300, 350, 400],
    ttft_iterations: int = 5,
    p99_iterations: int = 100,
):
    """Run Case 4 benchmark for TTFT and P99"""
    
    logger.info("=" * 80)
    logger.info("🚀 CASE 4: JIT + ASYNC ROLLING CUDA GRAPH BENCHMARK")
    logger.info("=" * 80)
    ...
    logger.info("Architecture: Two processes (JIT + CUDA Graph Generator + Context Creator)")
    logger.info("=" * 80)
    
    # Check if server is running
    if not os.path.exists(socket_path):
        logger.error(f"❌ Server not running. Socket file not found: {socket_path}")
        logger.error("   Start the server first: python case4_jit_cuda_generator.py")
        return
    
    # Initialize client
    client = JITAsyncCUDAClient(socket_path)
    ...
    for prompt_len in prompt_lengths:
        ...
        for iteration in range(ttft_iterations):
            ...
            prompt = tokenize_prompt(client.tokenizer, prompt_text, prompt_len)
    ...
    ttft_df.to_csv('output/case4_jit_cuda_ttft.csv', index=False)
    
    p99_df = pd.DataFrame(p99_results)
    p99_df.to_csv('output/case4_jit_cuda_p99.csv', index=False)
    
    logger.info("=" * 80)
    logger.info("📊 CASE 4 JIT + ASYNC CUDA GRAPH BENCHMARK SUMMARY")
    logger.info("=" * 80)
    ...
    logger.info("=" * 80)
    logger.info("✅ Case 4 JIT + Async CUDA Graph benchmark completed!")
    logger.info("📁 Results saved to: output/case4_jit_cuda_ttft.csv, output/case4_jit_cuda_p99.csv")
    
    return ttft_results, p99_results

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Case 4: JIT + Async CUDA Graph Client Benchmark")
